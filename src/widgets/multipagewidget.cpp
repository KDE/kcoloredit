...
/*
  Added support for icons and main title (name) by:
    ...
    : QGroupBox(parent)
{
    ...
    if (!name.isEmpty())
        hlayout->addWidget(new QLabel(name, this), Qt::AlignLeft);

    hlayout->addWidget(comboBox, Qt::AlignJustify);
...
void MultiPageWidget::insertPage(int index, QWidget *page)
{
    ...
    comboBox->insertItem(index, page->windowIcon(), title);
}
